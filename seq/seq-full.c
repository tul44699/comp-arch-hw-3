...
long long gen_instr_valid()
{
    return ((icode) == (I_NOP) || (icode) == (I_HALT) || (icode) == 
      ...
      (icode) == (I_POPQ) || (icode) == (I_IADDQ) || (icode) == (I_ISUBQ)
       || (icode) == (I_TJXX) || (icode) == (I_SHAQ) || (icode) == (I_DIVQ)
      );
}
...
long long gen_need_regids()
{
    return ((icode) == (I_RRMOVQ) || (icode) == (I_ALU) || (icode) == 
      ...
      (icode) == (I_ISUBQ) || (icode) == (I_TJXX) || (icode) == (I_SHAQ)
       || (icode) == (I_DIVQ));
}
...
long long gen_need_valC()
{
    return ((icode) == (I_IRMOVQ) || (icode) == (I_RMMOVQ) || (icode) == 
      (I_MRMOVQ) || (icode) == (I_JMP) || (icode) == (I_CALL) || (icode)
       == (I_IADDQ) || (icode) == (I_ISUBQ) || (icode) == (I_TJXX));
}
...
long long gen_srcA()
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_RMMOVQ) || (icode) == 
        ...
        (icode) == (I_RET)) ? (REG_RSP) : ((icode) == (I_TJXX) || (icode)
         == (I_SHAQ) || (icode) == (I_DIVQ)) ? (ra) : (REG_NONE));
}

long long gen_srcB()
{
    return (((icode) == (I_ALU) || (icode) == (I_RMMOVQ) || (icode) == 
        (I_MRMOVQ) || (icode) == (I_IADDQ) || (icode) == (I_ISUBQ)) ? (rb)
       : ((icode) == (I_PUSHQ) || (icode) == (I_POPQ) || (icode) == 
        (I_CALL) || (icode) == (I_RET)) ? (REG_RSP) : ((icode) == (I_TJXX)
         || (icode) == (I_SHAQ) || (icode) == (I_DIVQ)) ? (rb) : (REG_NONE)
      );
}
...
long long gen_dstE()
{
    return ((((icode) == (I_RRMOVQ)) & (cond)) ? (rb) : ((icode) == 
        (I_IRMOVQ) || (icode) == (I_ALU) || (icode) == (I_IADDQ) || (icode)
         == (I_ISUBQ)) ? (rb) : ((icode) == (I_PUSHQ) || (icode) == 
        (I_POPQ) || (icode) == (I_CALL) || (icode) == (I_RET)) ? (REG_RSP)
       : ((icode) == (I_SHAQ) || (icode) == (I_DIVQ)) ? (rb) : (REG_NONE));
}
...
long long gen_aluA()
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_ALU)) ? (vala) : (
        ...
        (I_MRMOVQ) || (icode) == (I_IADDQ) || (icode) == (I_ISUBQ)) ? 
      (valc) : ((icode) == (I_CALL) || (icode) == (I_PUSHQ)) ? -8 : ((icode)
         == (I_RET) || (icode) == (I_POPQ)) ? 8 : ((icode) == (I_TJXX)) ? 
      (valc) : ((icode) == (I_SHAQ) || (icode) == (I_DIVQ)) ? (vala) : 0);
}
...
long long gen_aluB()
{
    return (((icode) == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == 
        ...
        (icode) == (I_ISUBQ)) ? (valb) : ((icode) == (I_RRMOVQ) || (icode)
         == (I_IRMOVQ)) ? 0 : ((icode) == (I_TJXX) || (icode) == (I_SHAQ)
         || (icode) == (I_DIVQ)) ? (valb) : 0);
}
...
long long gen_alufun()
{
    return (((icode) == (I_ALU)) ? (ifun) : ((icode) == (I_ISUBQ)) ? 
      (A_SUB) : ((icode) == (I_TJXX)) ? (A_ADD) : ((icode) == (I_SHAQ)) ? 
      (A_SHFT) : ((icode) == (I_DIVQ)) ? (A_DIV) : (A_ADD));
}

long long gen_rem_cycles()
{
    return ((((icode) == (I_ALU)) & ((ifun) == (A_MUL))) ? 4 : 1);
}

long long gen_set_cc()
{
    return ((icode) == (I_ALU) || (icode) == (I_IADDQ) || (icode) == 
      (I_ISUBQ) || (icode) == (I_SHAQ) || (icode) == (I_DIVQ));
}
...
long long gen_Stat()
{
    return (((imem_error) | (dmem_error)) ? (STAT_ADR) : !(instr_valid) ? 
      (STAT_INS) : ((icode) == (I_HALT)) ? (STAT_HLT) : (((icode) == 
          (I_DIVQ)) & ((vala) == 0)) ? (STAT_HLT) : (STAT_AOK));
}

long long gen_new_pc()
{
    return (((((icode) == (I_TJXX)) & (tjxx_cond)) & (tjxx_addr_valid)) ? 
      (vale) : ((icode) == (I_TJXX)) ? (valp) : ((icode) == (I_CALL)) ? 
      (valc) : (((icode) == (I_JMP)) & (cond)) ? (valc) : ((icode) == 
        (I_RET)) ? (valm) : (valp));
}
