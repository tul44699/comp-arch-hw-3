...
bool_t tjxx_cond = FALSE;
bool_t tjxx_addr_valid = FALSE;
word_t ra = REG_NONE;
word_t rb = REG_NONE;
word_t valc = 0;
word_t valp = 0;
...
static char *format_prev() {
  ...
  sprintf(status_msg, "%c %s %s %s %s", prev_bcond ? 'Y' : 'N',
          iname(HPACK(prev_icode, prev_ifun)), istring, mstring, pstring);
...
static char *format_f() {
  ...
  sprintf(status_msg, "%s %s %s %s %s", iname(HPACK(icode, ifun)), reg_name(ra),
          reg_name(rb), valcstring, valpstring);
  return status_msg;
}
...
static char *format_d() {
  ...
  sprintf(status_msg, "%s %s %s %s %s %s", valastring, valbstring,
          reg_name(destE), reg_name(destM), reg_name(srcA), reg_name(srcB));
...
static char *format_e() {
  ...
  sprintf(status_msg, "%c %s", bcond ? 'Y' : 'N', valestring);
  return status_msg;
}
...
static byte_t sim_step(word_t alu_rem_cycles, bool_t *mult_cycle) {
  word_t aluA;
  word_t aluB;
  word_t alufun;
  ...
  aluB = gen_aluB();
  alufun = gen_alufun();
  vale = compute_alu(alufun, aluA, aluB, mult_cycle);
  cc_in = cc;
  if (gen_set_cc())
  ...
  if (icode == I_TJXX) {
    ...
    long long s_valE = (long long)vale;
    tjxx_addr_valid = s_valE >= 0 && s_valE <= 0xFFFF;
  }
...
word_t sim_run(word_t max_instr, byte_t *statusp, cc_t *ccp) {
  ...
  while (icount < max_instr) {
    ...
    run_status = sim_step(alu_rem_cycles, &mult_cycle);
    if (mult_cycle)
      MULT =
...
int simRunCmd(ClientData clientData, Tcl_Interp *interp, int argc,
  ...
  if (argc >= 2 &&
      (sscanf(argv[1], "%lld", &step_limit) != 1 || step_limit < 0)) {
