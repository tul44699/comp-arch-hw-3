...
#ifdef HAS_GUI
void addAppCommands(
    Tcl_Interp *interp); /* Add application-dependent commands */
#endif                   /* HAS_GUI */
...
#ifndef HAS_GUI
    printf("To run in GUI mode, you must recompile with the HAS_GUI constant "
           "defined.\n");
    exit(1);
#endif /* HAS_GUI */
...
static void run_tty_sim() {
  ...
  if (do_check) {
    ...
    for (step = 0; step < instr_limit && e == STAT_AOK; step++) {
      e = step_state(isa_state, stdout, &mult_cycle);
      if (mult_cycle)
        step = step + 3;
    }
...
static void usage(char *name) {
  ...
  printf("file.yo arg required in GUI mode, optional in TTY mode (default "
         "stdin)\n");
  printf("   -h     Print this message\n");
  printf("   -g     Run in GUI mode instead of TTY mode (default TTY)\n");
  printf(
      "   -l m   Set instruction limit to m [TTY mode only] (default %lld)\n",
      instr_limit);
  printf("   -v n   Set verbosity level to 0 <= n <= 2 [TTY mode only] "
         "(default %d)\n",
         verbosity);
...
static char *format_if_id(if_id_ptr state) {
  ...
  sprintf(status_msg, "%s %s %s %s %s %s", stat_name(state->status),
          iname(HPACK(state->icode, state->ifun)), reg_name(state->ra),
          reg_name(state->rb), valcstring, valpstring);
  return status_msg;
}
...
static char *format_id_ex(id_ex_ptr state) {
  ...
  sprintf(status_msg, "%s %s %s %s %s %s %s %s %s", stat_name(state->status),
          iname(HPACK(state->icode, state->ifun)), valcstring, valastring,
          valbstring, reg_name(state->deste), reg_name(state->destm),
          reg_name(state->srca), reg_name(state->srcb));
  return status_msg;
}
...
static char *format_ex_mem(ex_mem_ptr state) {
  ...
  wstring(state->vala, 4, 64, valastring);
  sprintf(status_msg, "%s %s %c %s %s %s %s", stat_name(state->status),
          iname(HPACK(state->icode, state->ifun)),
          state->takebranch ? 'Y' : 'N', valestring, valastring,
          reg_name(state->deste), reg_name(state->destm));
...
static char *format_mem_wb(mem_wb_ptr state) {
  ...
  sprintf(status_msg, "%s %s %s %s %s %s", stat_name(state->status),
          iname(HPACK(state->icode, state->ifun)), valestring, valmstring,
          reg_name(state->deste), reg_name(state->destm));
...
#ifdef HAS_GUI
  if (gui_mode) {
    report_pc(f_pc, pc_curr->status != STAT_BUB, if_id_curr->stage_pc,
              if_id_curr->status != STAT_BUB, id_ex_curr->stage_pc,
              id_ex_curr->status != STAT_BUB, ex_mem_curr->stage_pc,
              ex_mem_curr->status != STAT_BUB, mem_wb_curr->stage_pc,
              mem_wb_curr->status != STAT_BUB);
...
void tty_report(word_t cyc) {
  ...
  sim_log("D: instr = %s, rA = %s, rB = %s, valC = 0x%llx, valP = 0x%llx, Stat "
          "= %s\n",
          iname(HPACK(if_id_curr->icode, if_id_curr->ifun)),
          reg_name(if_id_curr->ra), reg_name(if_id_curr->rb), if_id_curr->valc,
          if_id_curr->valp, stat_name(if_id_curr->status));

  sim_log(
      "E: instr = %s, rem_cycles=0x%llx, valC = 0x%llx, valA = 0x%llx, valB = "
      "0x%llx\n   srcA = %s, srcB = %s, dstE = %s, dstM = %s, Stat = %s\n",
      iname(HPACK(id_ex_curr->icode, id_ex_curr->ifun)), id_ex_curr->rem_cycles,
  ...
  sim_log("W: instr = %s, valE = 0x%llx, valM = 0x%llx, dstE = %s, dstM = %s, "
          "Stat = %s\n",
...
static byte_t sim_step_pipe(word_t max_instr, word_t ccount,
                            word_t alu_rem_cycles, bool_t *mult_cycle) {
  ...
  do_if_stage();
  do_mem_stage();
  do_ex_stage(alu_rem_cycles, mult_cycle);
  do_id_wb_stages();
...
word_t sim_run_pipe(word_t max_instr, word_t max_cycle, byte_t *statusp,
  ...
  word_t icount = 0;
  word_t ccount = 0;
  word_t alu_rem_cycles;
  bool_t mult_cycle = FALSE;
  bool_t MULT = FALSE;
  byte_t run_status = STAT_AOK;
  while (icount < max_instr && ccount < max_cycle) {
    if (!MULT)
      alu_rem_cycles = gen_rem_cycles();
    run_status =
        sim_step_pipe(max_instr - icount, ccount, alu_rem_cycles, &mult_cycle);
    if (mult_cycle)
      MULT =
          TRUE; // for instns after MULQ, MULT will stay true for 3 more cycles
    if (MULT) {
      if (alu_rem_cycles > 0)
        alu_rem_cycles--;
      else
...
int simResetCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                char *argv[]);

int simLoadCodeCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                   char *argv[]);

int simLoadDataCmd(ClientData clientData, Tcl_Interp *interp, int argc,
                   char *argv[]);

int simRunCmd(ClientData clientData, Tcl_Interp *interp, int argc,
              char *argv[]);

int simModeCmd(ClientData clientData, Tcl_Interp *interp, int argc,
               char *argv[]);
...
int simRunCmd(ClientData clientData, Tcl_Interp *interp, int argc,
  ...
  if (argc >= 2 &&
      (sscanf(argv[1], "%lld", &cycle_limit) != 1 || cycle_limit < 0)) {
...
void show_cc(cc_t cc) {
  int code;
  sprintf(tcl_msg, "setCC %d %d %d", GET_ZF(cc), GET_SF(cc), GET_OF(cc));
  code = Tcl_Eval(sim_interp, tcl_msg);
  if (code != TCL_OK) {
...
void show_cpi() {
  int code;
  double cpi = instructions > 0 ? (double)cycles / instructions : 1.0;
  sprintf(tcl_msg, "showCPI %lld %lld %.2f", cycles, instructions, (double)cpi);
  code = Tcl_Eval(sim_interp, tcl_msg);
  if (code != TCL_OK) {
...
void signal_sources() {
  int code;
  sprintf(tcl_msg, "showSources %s %s", rname[amux], rname[bmux]);
  code = Tcl_Eval(sim_interp, tcl_msg);
  if (code != TCL_OK) {
...
/* Provide mechanism for simulator to report which instructions are in
   which stages */
void report_pc(unsigned fpc, unsigned char fpcv, unsigned dpc,
               unsigned char dpcv, unsigned epc, unsigned char epcv,
               unsigned mpc, unsigned char mpcv, unsigned wpc,
               unsigned char wpcv) {
  int status;
  char addr[10];
  char code[12];
  ...
  Tcl_DStringStartSublist(&cmd);
  sprintf(code, "%s %s %s %s %s", fpcv ? "F" : "", dpcv ? "D" : "",
          epcv ? "E" : "", mpcv ? "M" : "", wpcv ? "W" : "");
  ...
  */
  ...
  if (!imem_error) {
    sim_log("\tFetch: f_pc = 0x%llx, imem_instr = %s, f_instr = %s\n", f_pc,
            iname(instr), iname(HPACK(if_id_next->icode, if_id_next->ifun)));
  }
...
void do_ex_stage(word_t alu_rem_cycles, bool_t *mult_cycle) {
  ...
  e_bcond = cond_holds(cc, id_ex_curr->ifun); // Branch resolution

  auto tjxx_cond = FALSE;
  auto tjxx_addr_valid = FALSE;
  if (id_ex_curr->icode == I_TJXX) {
    long long s_valA = (long long)id_ex_curr->vala;
    switch (id_ex_curr->ifun) {
    case C_YES:
      tjxx_cond = TRUE;
      break;
    case C_E:
    ...
    tjxx_addr_valid = ex_mem_next->vale >= 0 && ex_mem_next->vale <= 0xFFFF;
  }

  ex_mem_next->takebranch = e_bcond || (tjxx_cond && tjxx_addr_valid);
  ...
  word_t aluout = compute_alu(alufun, alua, alub, mult_cycle);
  ex_mem_next->vale = aluout;
  sim_log("\tExecute: ALU: %c 0x%llx 0x%llx --> 0x%llx\n", op_name(alufun),
          alua, alub, aluout);

  if (setcc) {
    cc_in = compute_cc(alufun, alua, alub, mult_cycle);
