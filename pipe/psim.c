...
static void run_tty_sim() 
{
	...
	for (step = 0; step < instr_limit && e == STAT_AOK; step++) {
	    e = step_state(isa_state, stdout, &mult_cycle); if(mult_cycle) step = step+3;
	}
...
void tty_report(word_t cyc) {
  ...
  sim_log("E: instr = %s, rem_cycles=0x%llx, valC = 0x%llx, valA = 0x%llx, valB = 0x%llx\n   srcA = %s, srcB = %s, dstE = %s, dstM = %s, Stat = %s\n",
...
static byte_t sim_step_pipe(word_t max_instr, word_t ccount, word_t alu_rem_cycles, bool_t *mult_cycle)
{
...
word_t sim_run_pipe(word_t max_instr, word_t max_cycle, byte_t *statusp, cc_t *ccp)
{
    ...
    while (icount < max_instr && ccount < max_cycle) {
		if(!MULT) alu_rem_cycles = gen_rem_cycles();
        run_status = sim_step_pipe(max_instr-icount, ccount, alu_rem_cycles, &mult_cycle);
		if(mult_cycle) MULT = TRUE; // for instns after MULQ, MULT will stay true for 3 more cycles
		if(MULT) {if(alu_rem_cycles > 0) alu_rem_cycles--; else MULT = FALSE;}
