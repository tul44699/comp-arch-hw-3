...
word f_pc = [
	...
	# Completion of RET instruction. Know the return address. 
	W_icode == IRET : W_valM;
...
bool need_valC =
	f_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IJXX, ICALL, IIADDQ, IISUBQ };
...
word d_srcB = [
	D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, IIADDQ, IISUBQ  } : D_rB;
...
word d_dstE = [
	D_icode in { IRRMOVQ, IIRMOVQ, IOPQ, IIADDQ, IISUBQ } : D_rB;
	D_icode in { IPUSHQ, IPOPQ, ICALL, IRET } : RRSP;
...
bool set_cc = E_icode in { IOPQ, IIADDQ, IISUBQ } &&
...
bool F_stall =
	...
	((E_icode == IOPQ && E_ifun == IMULQ) && (E_rem_cycles >0)) || E_icode in { IMRMOVQ, IPOPQ } &&
...
# At most one of these can be true.
bool E_stall = ((E_icode == IOPQ && E_ifun == IMULQ) && (E_rem_cycles >0));
bool E_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) ||
	# Conditions for a load/use hazard
...
bool M_bubble = ((E_icode == IOPQ && E_ifun == IMULQ) && (E_rem_cycles >0)) || m_stat in { SADR, SINS, SHLT } || W_stat in { SADR, SINS, SHLT };
