...
wordsig E_stat 'id_ex_curr->status'    # Instruction status 
wordsig E_rem_cycles 'id_ex_curr->rem_cycles'   #No of remaining cycles
...
wordsig e_stat 'ex_mem_next->status' 
...
word f_pc = [
	# Next_pc is now known. Set it correctly.
	M_icode == IJXX && !M_Cnd : M_valA;  # Branch not taken
	E_icode == ITJXX && !e_Cnd : E_valA;  # Branch not taken
	W_icode == IRET : W_valM;
	1 : F_predPC;
];
...
bool need_valC =
	f_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IJXX, ICALL, IIADDQ, IISUBQ, ITJXX };

# Predict next value of PC
word f_predPC = [
	f_icode in { IJXX, ICALL, ITJXX } : f_valC;
	1 : f_valP;
];
...
word d_srcA = [
	D_icode in { IRRMOVQ, IRMMOVQ, IOPQ, IPUSHQ, ISHAQ, IDIVQ, ITJXX  } : D_rA;
...
word d_srcB = [
	D_icode in { IOPQ, IRMMOVQ, IMRMOVQ, IIADDQ, IISUBQ, ISHAQ, IDIVQ, ITJXX } : D_rB;
...
word d_dstE = [
	D_icode in { IRRMOVQ, IIRMOVQ, IOPQ, IIADDQ, IISUBQ, ISHAQ, IDIVQ } : D_rB;
	D_icode in { IPUSHQ, IPOPQ, ICALL, IRET } : RRSP;
...
#** In decode stage, we have 3 instructions that may need addresses other than next inst: ICALL, IJXX, RET. 
#** For the first two, the code chooses the next inst
word d_valA = [
...
word aluA = [
	...
	E_icode in { IIRMOVQ, IRMMOVQ, IMRMOVQ, IIADDQ, IISUBQ } : E_valC;
	E_icode in { ICALL, IPUSHQ } : -8;
	E_icode in { IRET, IPOPQ } : 8;
  E_icode in { ITJXX } : E_valC;
  E_icode in { ISHAQ, IDIVQ } : E_valA;
...
word aluB = [
	E_icode in { IRMMOVQ, IMRMOVQ, IOPQ, ICALL, 
		     IPUSHQ, IRET, IPOPQ, IIADDQ, IISUBQ } : E_valB;
	E_icode in { IRRMOVQ, IIRMOVQ } : 0;
  E_icode in { ITJXX, ISHAQ, IDIVQ } : E_valB;
	# Other instructions don't need ALU
];

word e_stat = [
    E_icode == IDIVQ && E_valB == 0 : SHLT;
    1 : E_stat;
];
...
word alufun = [
  ...
  E_icode == ITJXX : ALUADD;
  E_icode == ISHAQ : ALUSHFT;
  E_icode == IDIVQ: ALUDIV;
	1 : ALUADD;
];

## Set the ALU function cycles; Will create the function gen_rem_cycles
word rem_cycles = [
	(E_icode == IOPQ) && (E_ifun == IMULQ) : 4;
	1 : 1;
];

## Should the condition codes be updated?
bool set_cc = E_icode in { IOPQ, IIADDQ, IISUBQ, ISHAQ, IDIVQ } &&
	# State changes only during normal operation
	e_stat != SHLT && !m_stat in { SADR, SINS, SHLT } && !W_stat in { SADR, SINS, SHLT };
...
bool F_bubble = 0;
bool F_stall =
    (((E_icode == IOPQ && E_ifun == IMULQ) || E_icode == IDIVQ) && (E_rem_cycles > 0)) ||
...
bool D_stall = 
	...
	(((E_icode == IOPQ && E_ifun == IMULQ) || E_icode == IDIVQ) && (E_rem_cycles >0)) || E_icode in { IMRMOVQ, IPOPQ } &&
	 E_dstM in { d_srcA, d_srcB };

bool D_bubble =
    ((E_icode == IJXX && !e_Cnd) || (E_icode == ITJXX && !e_Cnd)) ||
    ( !(E_icode in { IMRMOVQ, IPOPQ } && E_dstM in { d_srcA, d_srcB }) &&
      IRET in { D_icode, E_icode, M_icode }) ||
    e_stat == SHLT;
...
# At most one of these can be true.
bool E_stall = (((E_icode == IOPQ && E_ifun == IMULQ) || E_icode == IDIVQ) && (E_rem_cycles >0));
bool E_bubble =
	# Mispredicted branch
	(E_icode == IJXX && !e_Cnd) || (E_icode == ITJXX && !e_Cnd) ||
	# Conditions for a load/use hazard
...
bool M_bubble = (((E_icode == IOPQ && E_ifun == IMULQ) || E_icode == IDIVQ) && (E_rem_cycles >0)) || m_stat in { SADR, SINS, SHLT } || W_stat in { SADR, SINS, SHLT };
