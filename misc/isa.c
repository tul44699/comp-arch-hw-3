...
struct {
  ...
  int id;
    ...
    {"%r12", REG_R12}, {"%r13", REG_R13}, {"%r14", REG_R14}, {"----", REG_NONE},
    {"----", REG_ERR}};
...
instr_t instruction_set[] = {
    ...
    {"isubq", HPACK(I_ISUBQ, F_NONE), 10, I_ARG, 2, 8, R_ARG, 1, 0},
...
instr_t invalid_instr = {"XXX", 0, 0, NO_ARG, 0, 0, NO_ARG, 0, 0};
...
instr_ptr bad_instr() { return &invalid_instr; }
...
void clear_mem(mem_t m) { memset(m->contents, 0, m->len); }
...
int load_mem(mem_t m, FILE *infile, int report_error) {
  ...
  while (fgets(buf, LINELEN, infile)) {
    ...
    while (isxdigit((int)(ch = buf[cpos++])) &&
      ...
      if (bytepos >= m->len) {
        if (report_error) {
          fprintf(stderr, "Error reading file. Invalid address. 0x%llx\n",
                  bytepos);
          fprintf(stderr, "Line %d:%s\n", lineno, buf);
        }
        return 0;
...
mem_t init_reg() { return init_mem(128); }

void free_reg(mem_t r) { free_mem(r); }

mem_t copy_reg(mem_t oldr) { return copy_mem(oldr); }
...
struct {
  char symbol;
  int id;
} alu_table[A_NONE + 1] = {{'+', A_ADD}, {'-', A_SUB}, {'&', A_AND},
                           {'^', A_XOR}, {'*', A_MUL}, {'?', A_NONE}};
...
word_t compute_alu(alu_t op, word_t argA, word_t argB, bool_t *mult_cycle) {
  word_t val;
  *mult_cycle = 0;
  switch (op) {
  case A_ADD:
...
cc_t compute_cc(alu_t op, word_t argA, word_t argB, bool_t *mult_cycle) {
  word_t val = compute_alu(op, argA, argB, mult_cycle);
  bool_t zero = (val == 0);
...
stat_t step_state(state_ptr s, FILE *error_file, bool_t *mult_cycle) {
  word_t argA, argB;
  byte_t byte0 = 0;
  byte_t byte1 = 0;
  itype_t hi0;
  ...
  lo0 = LO4(byte0);
  *mult_cycle = FALSE;
  //*mult_cycle = ((hi0 == I_ALU) && (lo0 == A_MUL));
  ...
  case I_CALL:
    ...
    if (!set_word_val(s->m, val, ftpc)) {
      if (error_file)
        fprintf(error_file, "PC = 0x%llx, Invalid stack address 0x%llx\n",
                s->pc, val);
      return STAT_ADR;
    }
    s->pc = cval;
